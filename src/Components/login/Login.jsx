...
const Login = (e) => {
  ...
  const handelSubmit = async (e) => {
    ...
    try {
      ...
      if (status === 1) {
        ...
        setUser(data)
        localStorage.setItem("username", JSON.stringify({first_name:data.first_name,last_name:data.last_name}))
  ...
  useEffect(() => {
    const callback = (event) => {
      ...
      if (event.key === 'Enter') { console.log("first"); ref.current.click() }
    };
